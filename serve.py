#!/usr/bin/env python3
"""Local preview. Static files, correct MIME types, byte ranges for the reel.

    python3 serve.py           # http://localhost:8181, nothing cached
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def send_head(self):
        """Serve a single byte range when asked — Safari will not play a video without it."""
...
if __name__ == "__main__":
    with Server(("", PORT), functools.partial(Handler, directory=str(ROOT))) as srv:
        print(f"REGNUM → http://localhost:{PORT}" + ("  (GitHub Pages cache headers)" if PAGES else ""))
        srv.serve_forever()
